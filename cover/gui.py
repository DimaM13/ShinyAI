"""Shyni Cover GUI - RVC каверы/даббинг. Запуск: .\\.venv\\Scripts\\python.exe cover\\gui.py"""
import os
import queue
import sys
import threading
...
class CoverGui:
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    if payload:
                        messagebox.showinfo("Shyni Cover", f"Готово:\n{payload}")
                    else:
                        messagebox.showwarning("Shyni Cover", "Не получилось - смотри лог")
        except queue.Empty:
            pass
        self.root.after(200, self.poll_log)
    ...
    def start(self):
        ...
        if not video or not os.path.exists(video):
            messagebox.showwarning("Shyni Cover", "Выбери видео")
            return
        if not self.voice_var.get():
            messagebox.showwarning("Shyni Cover", "Выбери голос")
            return
